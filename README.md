...
そのため、このサーバーは **フロントのオリジンを許可する CORS ポリシーを設定済み**の状態で渡しています。<br>
（`src/Program.cs` と `src/appsettings.json` の `Cors:AllowedOrigins`）

「クラサバが別物だから、つなぐのに一手間いる」を体感するためのポイントですが、研修の本題ではないので最初から設定済みにしてあります。<br>
許可オリジンを足したいときは `appsettings.json` に追記してください。
...
> Repository 層が使っている EF Core（DB アクセス）の基本を、このプロジェクトの実コードで解説した入門資料を用意しています: [docs/entity-framework-入門.md](./docs/entity-framework-入門.md)
